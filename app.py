...
def load_boundary():
    ...
    try:
        # เพิ่ม User-Agent ป้องกันเซิร์ฟเวอร์บล็อก
...
def load_gas_stations():
    ...
    try:
        # เพิ่ม User-Agent สำคัญมาก! เซิร์ฟเวอร์ Overpass มักจะบล็อก default python-requests
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        r = requests.post(url, data={'data': query}, headers=headers, timeout=30)
        
        # เช็คก่อนว่าเซิร์ฟเวอร์ส่งข้อมูลมาปกติ (Status 200) หรือไม่
        if r.status_code == 200:
            data = r.json()
            return data.get('elements', [])
        else:
            st.sidebar.warning(f"⚠️ โหลดปั๊มน้ำมันไม่สำเร็จ: เซิร์ฟเวอร์ Overpass ปฏิเสธการเชื่อมต่อ (Status {r.status_code})")
            return []
    except Exception as e:
        st.sidebar.warning(f"⚠️ โหลดปั๊มน้ำมันไม่สำเร็จ: เกิดข้อผิดพลาดในการเชื่อมต่อเครือข่าย")
        return []
...
st.sidebar.error("""
**🔴 เสี่ยงสูง (High Risk):** กิจการอันตราย (เช่น น้ำแข็ง, สารเคมี, พลาสติก, แช่แข็ง, อบพืช) หรือ เครื่องจักร > 500 HP หรือ คนงาน > 100 คน
""")

st.sidebar.warning("""
**🟡 เสี่ยงปานกลาง (Medium):** กิจการโรงกลึง, โลหะ, ซักรีด, เฟอร์นิเจอร์, กระจก หรือ เครื่องจักร > 100 HP หรือ คนงาน > 30 คน
""")

st.sidebar.success("""
**🟢 เสี่ยงต่ำ (Low Risk):** กิจการขนาดเล็กทั่วไปที่ไม่ได้อยู่ในหมวดอันตราย และเครื่องจักร < 100 HP
""")
